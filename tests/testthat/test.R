...
# 3. Separately test the ExtImage and BioFormatsImage methods
# 4. Less urgent, error messages from the validate functions
library(terra)
...
#Img(sfe1, image_id = "Cellbound1_z3") <- toExtImage(getImg(sfe1, image_id = "Cellbound1_z3"))
...
test_that("Save and read Vizgen data with SpatRaster", {
    ...
    expect_setequal(list.files(fsave), c("assays", "colgeometries", "column_data", "coordinates",
                                         "images", "local_results", "OBJECT", "row_data", "rowgeometries"))
    ...
    # The images
    imgs1 <- lapply(imgData(sfe1)$data, values)
    imgs2 <- lapply(imgData(sfe2)$data, values)
...
test_that("Save and read sfe with other fields populated", {
    ...
    expect_setequal(list.files(fsave), 
                    c("assays", "colgeometries", "column_data", "coordinates", "images", 
                      "OBJECT", "reduced_dimensions", "row_data", "rowgeometries", 
                      "spatial_graphs"))
    ...
    expect_true(dir.exists(file.path(fsave, "reduced_dimensions", "attrs")))
